...
void CCharacter::Tick()
{
	...
	{
		const int HookedPlayer = m_Core.GetHookedPlayer();
...
bool CCharacter::IsSnappingCharacterInView(int SnappingClientID)
{
	...
	if(PlayerAndHookNotInView)
	{
		for(const auto &AttachedPlayerID : m_Core.m_AttachedPlayers)
		{
			...
			if(pOtherPlayer && pOtherPlayer->m_Core.GetHookedPlayer() == ID)
			{
...
void CCharacter::FillAntibot(CAntibotCharacterData *pData)
{
	...
	pData->m_HookedPlayer = m_Core.GetHookedPlayer();
...
void CCharacter::Pause(bool Pause)
{
	...
	if(Pause)
	{
		...
		if(m_Core.GetHookedPlayer() != -1) // Keeping hook would allow cheats
		{
			ResetHook();
			GameWorld()->ReleaseHooked(GetPlayer()->GetCID());
		}
