...
const float CCharacterCore::sc_PhysicalSize = 28.0f;
const vec2 CCharacterCore::sc_PhysicalSizeVec2 = vec2(sc_PhysicalSize, sc_PhysicalSize);
...
void CCharacterCore::Tick(bool UseInput)
{
	...
	// get ground state
	bool Grounded = false;
	if(m_pCollision->CheckPoint(m_Pos.x + sc_PhysicalSize / 2, m_Pos.y + sc_PhysicalSize / 2 + 5))
		Grounded = true;
	if(m_pCollision->CheckPoint(m_Pos.x - sc_PhysicalSize / 2, m_Pos.y + sc_PhysicalSize / 2 + 5))
		Grounded = true;
	...
	if(UseInput)
	{
		...
		if(m_Input.m_Hook)
		{
			if(m_HookState == HOOK_IDLE)
			{
				...
				m_HookPos = m_Pos + TargetDirection * sc_PhysicalSize * 1.5f;
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		if(this->m_Hook && m_pWorld && m_Tuning.m_PlayerHooking)
		{
			...
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				...
				{
					if(distance(pCharCore->m_Pos, ClosestPoint) < sc_PhysicalSize + 2.0f)
					{
		...
		if(m_HookState == HOOK_FLYING)
		{
			...
			if(GoingThroughTele && m_pWorld && m_pTeleOuts && !m_pTeleOuts->empty() && !(*m_pTeleOuts)[teleNr - 1].empty())
			{
				...
				m_HookPos = (*m_pTeleOuts)[teleNr - 1][RandomOut] + TargetDirection * sc_PhysicalSize * 1.5f;
				m_HookDir = TargetDirection;
	...
	if(m_pWorld)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(Distance > 0)
			{
				...
				if(CanCollide && Distance < sc_PhysicalSize * 1.25f && Distance > 0.0f)
				{
					float a = (sc_PhysicalSize * 1.45f - Distance);
					float Velocity = 0.5f;
				...
				{
					if(Distance > sc_PhysicalSize * 1.50f) // TODO: fix tweakable variable
					{
...
void CCharacterCore::Move()
{
	...
	vec2 OldVel = m_Vel;
	m_pCollision->MoveBox(&NewPos, &m_Vel, sc_PhysicalSizeVec2, 0);
