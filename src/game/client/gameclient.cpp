		...
		for(int i = 0; i < Num; i++)
		{
			...
			else if(Item.m_Type == NETOBJTYPE_INFCLASSPLAYER)
			{
				...
				if(Item.m_ID < MAX_CLIENTS)
				{
					...
					ProcessInfClassPlayerInfo(pClient, pPlayerData);
				}
			}
			else if(Item.m_Type == NETOBJTYPE_INFCLASSCLASSINFO)
			{
				const CNetObj_InfClassClassInfo *pClassInfo = (const CNetObj_InfClassClassInfo *)pData;

				if(Item.m_ID < MAX_CLIENTS)
				{
					CClientData *pClient = &m_aClients[Item.m_ID];
					ProcessInfClassClassInfo(pClient, pClassInfo);
				}
			}
			else if(Item.m_Type == NETOBJTYPE_INFCLASSGAMEINFO)
			{
				const CNetObj_InfClassGameInfo *pGameInfo = (const CNetObj_InfClassGameInfo *)pData;
...
void CGameClient::ProcessInfClassPlayerInfo(CClientData *pClient, const CNetObj_InfClassPlayer *pPlayerData)
{
...
void CGameClient::ProcessInfClassClassInfo(CClientData *pClient, const CNetObj_InfClassClassInfo *pClassInfo)
{
...
float CGameClient::GetAspectTextureRatio(DAMAGE_TYPE DamageType)
{
	int SpriteIndex = GetInfclassSpriteForDamageType(DamageType);
	if(SpriteIndex < 0)
		return 1;
...
void CGameClient::SnapCollectEntities()
{
	...
	for(int Index = 0; Index < NumSnapItems; Index++)
	{
		...
		else if(Item.m_Type == NETOBJTYPE_PICKUP || Item.m_Type == NETOBJTYPE_LASER || Item.m_Type == NETOBJTYPE_PROJECTILE || Item.m_Type == NETOBJTYPE_DDNETPROJECTILE || Item.m_Type == NETOBJTYPE_INFCLASSOBJECT)
