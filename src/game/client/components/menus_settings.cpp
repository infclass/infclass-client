...
static const int GenericKeysNumber = sizeof(gs_aKeys) / sizeof(gs_aKeys[0]);
...
void CMenus::DoSettingsControlsButtons(int Start, int Stop, CUIRect View, CUIRect ScopeView)
{
	for(int i = Start; i < Stop; i++)
	{
		...
		if(NewId != OldId || NewModifierCombination != OldModifierCombination)
		{
			...
			if(NewId != 0)
				Binds.Bind(NewId, gs_aKeys[i].m_pCommand, false, NewModifierCombination);
		}
...
void CMenus::RenderSettingsControls(CUIRect MainView)
{
	...
	if(DoButton_MenuTab((void *)&s_aPageTabs[0], Localize("General"), s_CurControlsTab == 0, &TabLabel1, 5, NULL, NULL, NULL, NULL, 4))
		s_CurControlsTab = 0;
	if(DoButton_MenuTab((void *)&s_aPageTabs[1], Localize("InfClass"), s_CurControlsTab == 1, &TabLabel2, 10, NULL, NULL, NULL, NULL, 4))
		s_CurControlsTab = 1;
...
void CMenus::RenderSettingsInfClassControls(CUIRect MainView)
{
	...
	// Report location settings
	{
		...
		TextRender()->Text(0, ReportLocationSettings.x, ReportLocationSettings.y + (14.0f + 5.0f + 10.0f - 14.0f * UI()->Scale()) / 2.f, 14.0f * UI()->Scale(), Localize("Report location"), -1.0f);

		ReportLocationSettings.HSplitTop(14.0f + 5.0f + 10.0f, 0, &ReportLocationSettings);

		DoSettingsControlsButtons(LastButton, LastButton + LocationKeys, ReportLocationSettings, MainView);
	...
	// Report location clearsettings
	{
		...
		TextRender()->Text(0, ReportLocationClearSettings.x, ReportLocationClearSettings.y + (14.0f + 5.0f + 10.0f - 14.0f * UI()->Scale()) / 2.f, 14.0f * UI()->Scale(), Localize("Report location is clear"), -1.0f);

		ReportLocationClearSettings.HSplitTop(14.0f + 5.0f + 10.0f, 0, &ReportLocationClearSettings);

		DoSettingsControlsButtons(LastButton, LastButton + LocationKeys, ReportLocationClearSettings, MainView);
		LastButton += LocationKeys;
	}

	{
		...
		TextRender()->Text(0, TeamChatSettings.x, TeamChatSettings.y + (14.0f + 5.0f + 10.0f - 14.0f * UI()->Scale()) / 2.f, 14.0f * UI()->Scale(), Localize("Team chat"), -1.0f);

		TeamChatSettings.HSplitTop(14.0f + 5.0f + 10.0f, 0, &TeamChatSettings);

		DoSettingsControlsButtons(LastButton, LastButton + TeamChatKeys, TeamChatSettings, MainView);
...

	{
		...
		TextRender()->Text(0, ExtraCommandsSettings.x, ExtraCommandsSettings.y + (14.0f + 5.0f + 10.0f - 14.0f * UI()->Scale()) / 2.f, 14.0f * UI()->Scale(), Localize("Extra commands"), -1.0f);

		ExtraCommandsSettings.HSplitTop(14.0f + 5.0f + 10.0f, 0, &ExtraCommandsSettings);

		DoSettingsControlsButtons(LastButton, LastButton + ExtraCommandKeys, ExtraCommandsSettings, MainView);
